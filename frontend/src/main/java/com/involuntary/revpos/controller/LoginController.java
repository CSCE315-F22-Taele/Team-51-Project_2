...
public class LoginController implements Initializable {
    ...
    public static void openModal(Scene posScene) throws IOException {
        Parent root = FXMLLoader.load(PosController.class.getResource("/views/managerLogin.fxml"));
        ...
        modal.getIcons().add(new Image(LoginController.class.getResourceAsStream("/images/box.png")));
    ...
    public static void verifyManagerLogin(Scene posScene) {
        ...
        try {
            ...
            String loginData = "SELECT count(1) FROM accounts WHERE type = 'manager' AND username = '" + referEmployeeUsername.getText() + "' AND password = '" + referEmployeePassword.getText() + "'";
            statement = dbConnection.createStatement();
            queryResult = statement.executeQuery(loginData);

            while(queryResult.next()) {
                if(queryResult.getInt(1) == 1) {
                    Stage stage = (Stage) (referManagerLoginButton.getScene().getWindow());
                    stage.close();
                    Parent root = FXMLLoader.load(PosController.class.getResource("/views/settings.fxml"));
